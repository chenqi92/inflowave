...
import { Search, RefreshCw, Settings, Database as DatabaseIcon } from 'lucide-react';
...
export const VersionAwareDatabaseExplorer: React.FC<VersionAwareDatabaseExplorerProps> = ({
  connectionId,
  onTableSelect,
  onQueryGenerate,
  className = '',
    ...
    switch (databaseType) {
      case 'InfluxDB':
        if (node.nodeType === 'measurement') {
          // 从节点路径中提取数据库和保留策略信息
          query = `SELECT * FROM "${node.name}" LIMIT 100`;
        }
        break;
      case 'IoTDB':
        if (node.nodeType === 'timeseries') {
          query = `SELECT ${node.name} FROM root.** LIMIT 100`;
            ...
            <CardContent className="pt-0">
              <div className="space-y-2 text-xs">
                ...
                <div className="flex space-x-2 mt-3">
                  {(selectedNode.nodeType === 'measurement' || selectedNode.nodeType === 'timeseries') && (
                    <Button
                      size="sm"
                      variant="outline"
