...
export const UnifiedQueryHistory: React.FC<UnifiedQueryHistoryProps> = ({
  ...
  const renderHistoryItem = (item: QueryHistoryItem) => (
    <Card key={item.id} className="mb-3 hover:shadow-md transition-shadow border-l-4 border-l-transparent hover:border-l-primary/30">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 space-y-3">
            ...
            <div className="space-y-2">
              <div className="text-sm text-muted-foreground line-clamp-2 font-mono bg-muted/30 p-2 rounded-md">
                {item.query}
              </div>
            ...
            {item.error && (
              <div className="mt-2 p-3 bg-destructive/5 border border-destructive/20 rounded-md text-sm text-destructive">
          ...
          <div className="flex items-center gap-1">
            ...
            {mode === 'modal' && (
              ...
              >
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant="ghost" size="sm" className="h-8 w-8 hover:bg-destructive/10 hover:text-destructive transition-colors">
  ...
  const renderSavedQuery = (query: SavedQuery) => (
    <Card key={query.id} className={`${mode === 'page' ? '' : 'mb-3'} hover:shadow-md transition-shadow border-l-4 ${query.favorite ? 'border-l-yellow-400' : 'border-l-transparent hover:border-l-primary/30'}`}>
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 space-y-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <DropdownMenuContent>
                ...
                {features.favoriteQueries && (
                  <DropdownMenuItem onClick={() => toggleFavorite(query.id)}>
                    <Star className={`w-4 h-4 mr-2 ${query.favorite ? 'text-yellow-500 fill-current' : ''}`} />
    ...
    <div className={mode === 'page' ? 'h-full flex flex-col' : 'h-[600px] flex flex-col'}>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1 min-w-0">
              <div className="relative flex items-center">
                <Search className="absolute left-3 z-10 w-4 h-4 text-muted-foreground pointer-events-none" />
                <Input
                  placeholder="搜索查询..."
                  value={searchText}
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'history' | 'saved')} className="flex-1 flex flex-col">
        ...
        <TabsContent value="saved" className="flex-1 mt-0">
          <ScrollArea className="h-full">
            <div className={mode === 'page' ? 'p-4 grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4' : 'p-4 space-y-3'}>
              {loading ? (
