...
const Menu = React.forwardRef<HTMLDivElement, MenuProps>(
  ({ className, children, mode = "vertical", theme = "light", selectedKeys = [], onSelect, items, ...props }, ref) => {
    const renderItem = (item: MenuItem, level = 0) => {
      ...
      if (item.type === "group") {
        return (
          <div key={item.key} className="px-3 py-2 text-xs font-medium text-muted-foreground uppercase tracking-wider">
            {item.label}
          </div>
        )
      }
