...
const Row = React.forwardRef<HTMLDivElement, RowProps>(
  ({ className, children, gutter, justify = "start", align = "start", ...props }, ref) => {
    const gutterValue = Array.isArray(gutter) ? gutter : [gutter || 0, gutter || 0]
...
const Col = React.forwardRef<HTMLDivElement, ColProps>(
  ({ 
    ...
    const style = {
      flex: flex ? (typeof flex === 'number' ? `${flex} ${flex} auto` : flex) : undefined
    }
