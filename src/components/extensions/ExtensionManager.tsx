...
const ExtensionManager: React.FC = () => {
  ...
  const loadApiIntegrations = async () => {
    try {
      const result = await safeTauriInvoke<APIIntegration[]>(
  ...
  const loadAutomationRules = async () => {
    try {
      const result = await safeTauriInvoke<AutomationRule[]>(
  ...
  const testApiIntegration = async (integration: APIIntegration) => {
    setLoading(true);
    try {
      const result = await safeTauriInvoke('test_api_integration', {
        integration,
      });
  ...
  const toggleApiIntegration = async (
    integrationId: string,
    enabled: boolean
  ) => {
    try {
      await safeTauriInvoke('toggle_api_integration', {
        integrationId,
        enabled,
      });
  ...
  const toggleWebhook = async (webhookId: string, enabled: boolean) => {
    try {
      await safeTauriInvoke('toggle_webhook', { webhookId, enabled });
  ...
  const toggleAutomationRule = async (ruleId: string, enabled: boolean) => {
    try {
      await safeTauriInvoke('toggle_automation_rule', { ruleId, enabled });
    ...
    <div className='p-6 space-y-6'>
                      ...
                      <div className='flex items-center justify-between'>
                        <div className='flex-1'>
                          <div className='flex items-center gap-2 mb-2'>
                            ...
                            <Badge
                              variant={
                                plugin.enabled ? 'default' : 'destructive'
                              }
                            >
                      ...
                      <div className='flex items-center justify-between'>
                        ...
                        <div className='flex items-center space-x-2 ml-4'>
                          ...
                          <Switch
                            ...
                            onCheckedChange={checked =>
                              toggleApiIntegration(integration.id, checked)
                            }
                          />
                        </div>
                      </div>
      ...
      <Dialog open={apiModalOpen} onOpenChange={setApiModalOpen}>
        <DialogContent className='sm:max-w-[600px]'>
          ...
          <Form {...apiForm}>
            <form
              onSubmit={apiForm.handleSubmit(createApiIntegration)}
              className='space-y-4'
            >
              <FormField
      ...
      <Dialog open={webhookModalOpen} onOpenChange={setWebhookModalOpen}>
        <DialogContent className='sm:max-w-[600px]'>
          ...
          <Form {...webhookForm}>
            <form
              onSubmit={webhookForm.handleSubmit(createWebhook)}
              className='space-y-4'
            >
              <FormField
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value='query_completed'>
                            查询完成
                          </SelectItem>
                          <SelectItem value='connection_status'>
                            连接状态变化
                          </SelectItem>
                          <SelectItem value='export_completed'>
                            导出完成
                          </SelectItem>
                          <SelectItem value='alert_triggered'>
                            警报触发
                          </SelectItem>
                        </SelectContent>
                      </Select>
      ...
      <Dialog open={automationModalOpen} onOpenChange={setAutomationModalOpen}>
        <DialogContent className='sm:max-w-[700px]'>
          ...
          <Form {...automationForm}>
            <form
              onSubmit={automationForm.handleSubmit(createAutomationRule)}
              className='space-y-4'
            >
              <FormField
