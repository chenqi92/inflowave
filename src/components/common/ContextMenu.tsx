      ...
      case 'query-editor':
        return [
          ...
          },
          {
            ...
            icon: <Copy className="w-4 h-4" />,
            action: { type: 'copyToClipboard', text: target.hasSelection ? target.selectedText : target.query }
          },
          { id: 'sep2', type: 'separator' },
          {
            id: 'query_templates',
            ...
            children: [
              ...
              },
              {
                ...
                label: '按时间范围查询',
                action: { type: 'insertTemplate', template: 'SELECT * FROM measurement_name WHERE time >= now() - 1h' }
              },
              {
                id: 'template_aggregation',
                label: '聚合查询',
                action: { type: 'insertTemplate', template: 'SELECT MEAN(field_name) FROM measurement_name WHERE time >= now() - 1h GROUP BY time(5m)' }
              },
              {
