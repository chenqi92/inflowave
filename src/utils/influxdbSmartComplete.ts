...
export interface SmartSuggestion {
  label: string;
  insertText: string;
  type: 'keyword' | 'function' | 'database' | 'measurement' | 'field' | 'tag' | 'template' | 'operator' | 'value' | 'constant';
  priority: number;
  description: string;
...
const INFLUXDB_SYNTAX = {
  ...
  SHOW_COMMANDS: {
    ...
    'TAG VALUES': { desc: '显示标签值', example: 'SHOW TAG VALUES FROM cpu WITH KEY = "host"' },
    'RETENTION POLICIES': { desc: '显示保留策略', example: 'SHOW RETENTION POLICIES ON mydb' },
    'CONTINUOUS QUERIES': { desc: '显示连续查询', example: 'SHOW CONTINUOUS QUERIES' },
  ...
  TRANSFORM_FUNCTIONS: {
    ...
    'NON_NEGATIVE_DERIVATIVE': { desc: '非负导数', example: 'NON_NEGATIVE_DERIVATIVE(field, 1s)' },
    'NON_NEGATIVE_DIFFERENCE': { desc: '非负差值', example: 'NON_NEGATIVE_DIFFERENCE(field)' },
  ...
  PREDICTION_FUNCTIONS: {
    'HOLT_WINTERS': { desc: 'Holt-Winters预测', example: 'HOLT_WINTERS(field, 10, 4)' },
    'HOLT_WINTERS_WITH_FIT': { desc: 'Holt-Winters带拟合', example: 'HOLT_WINTERS_WITH_FIT(field, 10, 4)' }
  },

  // 技术分析函数
  TECHNICAL_ANALYSIS: {
    'CHANDE_MOMENTUM_OSCILLATOR': { desc: 'Chande动量振荡器', example: 'CHANDE_MOMENTUM_OSCILLATOR(field, 10)' },
    'EXPONENTIAL_MOVING_AVERAGE': { desc: '指数移动平均', example: 'EXPONENTIAL_MOVING_AVERAGE(field, 10)' },
    'DOUBLE_EXPONENTIAL_MOVING_AVERAGE': { desc: '双指数移动平均', example: 'DOUBLE_EXPONENTIAL_MOVING_AVERAGE(field, 10)' },
    'KAUFMANS_EFFICIENCY_RATIO': { desc: 'Kaufman效率比率', example: 'KAUFMANS_EFFICIENCY_RATIO(field, 10)' },
    'KAUFMANS_ADAPTIVE_MOVING_AVERAGE': { desc: 'Kaufman自适应移动平均', example: 'KAUFMANS_ADAPTIVE_MOVING_AVERAGE(field, 10)' },
    'TRIPLE_EXPONENTIAL_MOVING_AVERAGE': { desc: '三重指数移动平均', example: 'TRIPLE_EXPONENTIAL_MOVING_AVERAGE(field, 10)' },
    'TRIPLE_EXPONENTIAL_DERIVATIVE': { desc: '三重指数导数', example: 'TRIPLE_EXPONENTIAL_DERIVATIVE(field, 10)' },
    'RELATIVE_STRENGTH_INDEX': { desc: '相对强弱指数', example: 'RELATIVE_STRENGTH_INDEX(field, 10)' }
  },
  ...
  // 数据写入
  INSERT_KEYWORDS: {
    'INSERT': { desc: '插入数据', example: 'INSERT INTO measurement VALUES (...)' },
  ...
  DATABASE_MANAGEMENT: {
    ...
    'RETENTION POLICY': { desc: '保留策略', example: 'CREATE RETENTION POLICY "policy" ON "db"' },
    'CONTINUOUS QUERY': { desc: '连续查询', example: 'CREATE CONTINUOUS QUERY "cq" ON "db"' },
    'USER': { desc: '用户', example: 'CREATE USER "username" WITH PASSWORD "password"' },
    'SUBSCRIPTION': { desc: '订阅', example: 'CREATE SUBSCRIPTION "sub" ON "db"."rp"' }
  },
...
const QUERY_TEMPLATES = [
  ...
  {
    label: '分组查询',
    insertText: 'SELECT ${1:field} FROM "${2:measurement}" WHERE time > now() - ${3:1h} GROUP BY ${4:tag}',
    description: '按标签分组查询',
    category: 'query'
  },
  {
    label: '多字段查询',
    insertText: 'SELECT ${1:field1}, ${2:field2}, ${3:field3} FROM "${4:measurement}" WHERE time > now() - ${5:1h} LIMIT ${6:100}',
    description: '多字段数据查询',
    category: 'query'
  },
  {
    label: '条件过滤查询',
    insertText: 'SELECT * FROM "${1:measurement}" WHERE time > now() - ${2:1h} AND ${3:field} ${4:>} ${5:value}',
    description: '带条件过滤的查询',
    category: 'query'
  },
  {
    label: '排序查询',
    insertText: 'SELECT * FROM "${1:measurement}" WHERE time > now() - ${2:1h} ORDER BY time ${3:DESC} LIMIT ${4:100}',
    description: '排序后的结果查询',
    category: 'query'
  },
  {
    label: '正则表达式查询',
    insertText: 'SELECT * FROM "${1:measurement}" WHERE ${2:tag} =~ /${3:pattern}/ AND time > now() - ${4:1h}',
  ...
  {
    label: '显示标签值',
    insertText: 'SHOW TAG VALUES FROM "${1:measurement}" WITH KEY = "${2:tag_key}"',
  ...
  {
    label: '创建保留策略',
    insertText: 'CREATE RETENTION POLICY "${1:policy_name}" ON "${2:database}" DURATION ${3:30d} REPLICATION ${4:1}${5: DEFAULT}',
...
class InfluxDBSmartCompleteEngine {
  ...
  private detectCurrentClause(text: string): string {
    const upperText = text.toUpperCase();
    const clauses = ['ORDER BY', 'GROUP BY', 'WHERE', 'FROM', 'SELECT', 'INTO', 'VALUES'];
    ...
    for (const clause of clauses) {
      ...
      if (lastIndex !== -1) {
        ...
        const hasLaterClause = clauses.some(c => c !== clause && afterClause.includes(c));
        if (!hasLaterClause) {
  ...
  private expectsOperator(text: string): boolean {
    return /\b\w+\s*$/.test(text) && !/\b(WHERE|AND|OR|SELECT|FROM|GROUP|ORDER)\s+\w*$/.test(text.toUpperCase());
  }
  ...
  private extractMeasurements(text: string): string[] {
    ...
    return matches ? matches.map(m => m.replace(/FROM\s+["`]?([^"`\s,]+)["`]?/i, '$1')) : [];
  }
   ...
   * 确保缓存是最新的
   */
  private async ensureFreshCache(connectionId: string, database: string): Promise<void> {
   ...
   * 更新缓存
   */
  private async updateCache(connectionId: string, database: string): Promise<void> {
    try {
      // 获取测量名
      ...
      this.databaseCache.set(database, measurements.map(m => m.name));

      // 获取字段和标签信息
      for (const measurement of measurements.slice(0, 10)) { // 限制数量避免过多请求
        try {
  ...
  private async addContextualSuggestions(
    ...
    switch (context.currentClause) {
      case 'FROM':
        this.addMeasurementSuggestions(suggestions, database, context.currentWord);
        break;
      case 'WHERE':
  ...
  private addBasicKeywords(suggestions: SmartSuggestion[]): void {
    Object.entries(INFLUXDB_SYNTAX.BASIC_KEYWORDS).forEach(([keyword, info]) => {
      suggestions.push({
        label: keyword,
        insertText: keyword + ' ',
        type: 'keyword',
    ...
    Object.entries(INFLUXDB_SYNTAX.ADMIN_KEYWORDS).forEach(([keyword, info]) => {
      suggestions.push({
        label: keyword,
        insertText: keyword + ' ',
        type: 'keyword',
   ...
   * 添加匹配的关键词
   */
  private addMatchingKeywords(suggestions: SmartSuggestion[], prefix: string): void {
    if (!prefix) return;
    ...
    const syntaxCategories = [
      { data: INFLUXDB_SYNTAX.BASIC_KEYWORDS, type: 'keyword', priority: 95, category: 'query' },
      { data: INFLUXDB_SYNTAX.ADMIN_KEYWORDS, type: 'keyword', priority: 90, category: 'admin' },
      { data: INFLUXDB_SYNTAX.SHOW_COMMANDS, type: 'keyword', priority: 85, category: 'admin' },
      { data: INFLUXDB_SYNTAX.AGGREGATE_FUNCTIONS, type: 'function', priority: 80, category: 'function' },
      { data: INFLUXDB_SYNTAX.SELECTOR_FUNCTIONS, type: 'function', priority: 80, category: 'function' },
      { data: INFLUXDB_SYNTAX.TRANSFORM_FUNCTIONS, type: 'function', priority: 75, category: 'function' },
      { data: INFLUXDB_SYNTAX.PREDICTION_FUNCTIONS, type: 'function', priority: 70, category: 'function' },
      { data: INFLUXDB_SYNTAX.TECHNICAL_ANALYSIS, type: 'function', priority: 70, category: 'function' },
      { data: INFLUXDB_SYNTAX.LOGICAL_OPERATORS, type: 'operator', priority: 85, category: 'operator' },
      { data: INFLUXDB_SYNTAX.TIME_KEYWORDS, type: 'keyword', priority: 80, category: 'time' },
      { data: INFLUXDB_SYNTAX.INSERT_KEYWORDS, type: 'keyword', priority: 75, category: 'insert' },
      { data: INFLUXDB_SYNTAX.DATABASE_MANAGEMENT, type: 'keyword', priority: 75, category: 'admin' },
      { data: INFLUXDB_SYNTAX.PRIVILEGE_KEYWORDS, type: 'keyword', priority: 70, category: 'admin' }
    ];
    ...
    syntaxCategories.forEach(({ data, type, priority, category }) => {
      Object.entries(data).forEach(([keyword, info]) => {
        if (keyword.toLowerCase().startsWith(lowerPrefix)) {
          const matchPriority = this.calculateMatchPriority(keyword, prefix, priority);
   ...
   * 计算匹配优先级
   */
  private calculateMatchPriority(keyword: string, prefix: string, basePriority: number): number {
  ...
  private async addSelectSuggestions(
    ...
    if (context.currentClause === 'SELECT') {
      ...
      if (context.measurements.length > 0) {
        await this.addFieldSuggestions(suggestions, context.measurements[0], database);
      }
    }
  }
      ...
      Object.entries(data).forEach(([func, info]) => {
        if (!prefix || func.toLowerCase().startsWith(prefix.toLowerCase())) {
          suggestions.push({
            ...
            insertText: `${func}($1)`,
            type: 'function',
            priority: prefix ? this.calculateMatchPriority(func, prefix, priority) : priority,
            description: info.desc,
   ...
   * 添加 SHOW 建议
   */
  private addShowSuggestions(suggestions: SmartSuggestion[], context: any): void {
    Object.entries(INFLUXDB_SYNTAX.SHOW_COMMANDS).forEach(([command, info]) => {
      if (!context.currentWord || command.toLowerCase().includes(context.currentWord.toLowerCase())) {
        suggestions.push({
          label: command,
          insertText: command,
          type: 'keyword',
   ...
   * 添加 CREATE 建议
   */
  private addCreateSuggestions(suggestions: SmartSuggestion[], context: any): void {
    Object.entries(INFLUXDB_SYNTAX.DATABASE_MANAGEMENT).forEach(([obj, info]) => {
      if (!context.currentWord || obj.toLowerCase().includes(context.currentWord.toLowerCase())) {
        suggestions.push({
          label: obj,
          insertText: obj + ' ',
          type: 'keyword',
   ...
   * 添加 DROP 建议
   */
  private addDropSuggestions(suggestions: SmartSuggestion[], context: any): void {
    const dropObjects = ['DATABASE', 'MEASUREMENT', 'SERIES', 'RETENTION POLICY', 'CONTINUOUS QUERY', 'USER'];
  ...
  private async addGroupBySuggestions(
    ...
    // 时间分组建议
    const timeGroups = ['time(1s)', 'time(10s)', 'time(1m)', 'time(5m)', 'time(15m)', 'time(1h)', 'time(1d)'];
   ...
   * 添加值建议
   */
  private addValueSuggestions(suggestions: SmartSuggestion[], context: any): void {
    // 布尔值
  ...
  private addOperatorSuggestions(suggestions: SmartSuggestion[]): void {
    Object.entries(INFLUXDB_SYNTAX.COMPARISON_OPERATORS).forEach(([op, info]) => {
      suggestions.push({
        label: op,
        insertText: ` ${op} `,
        type: 'operator',
   ...
   * 排序建议
   */
  private sortSuggestions(suggestions: SmartSuggestion[], context: any): SmartSuggestion[] {
