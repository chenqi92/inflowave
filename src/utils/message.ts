...
// 创建标准化的 Sonner 配置
const createToastOptions = (duration?: number, options?: Partial<ExternalToast>): ExternalToast => ({
...
export const showMessage = {
  ...
  custom: (content: string, options?: ExternalToast) => message.custom(content, options),
  promise: <T>(promise: Promise<T>, msgs: {
    loading: string;
    success: string | ((data: T) => string);
    error: string | ((error: any) => string);
...
export const specialMessage = {
  // 连接相关消息
  connectionSuccess: (name: string) => showNotification.success({
    message: "连接成功",
  ...
  connectionError: (name: string, error: string) => showNotification.error({
    message: "连接失败",
  ...
  connectionLost: (name: string) => showNotification.warning({
    message: "连接中断",
  ...
  // 查询相关消息
  querySuccess: (rowCount: number, duration: number) => showNotification.success({
    message: "查询完成",
    description: `返回 ${rowCount} 行数据，耗时 ${duration}ms`
  }),

  queryError: (error: string) => showNotification.error({
    message: "查询失败",
    description: error,
    duration: 5
  }),

  queryTimeout: (timeout: number) => showNotification.warning({
    message: "查询超时",
  ...
  // 导出相关消息
  exportSuccess: (format: string, filename?: string) => showNotification.success({
    message: "导出成功",
    description: filename ? `已导出为 ${filename}` : `已导出为 ${format.toUpperCase()} 格式`,
    action: {
      label: "打开文件夹",
  ...
  exportError: (error: string) => showNotification.error({
    message: "导出失败",
    description: error,
    action: {
      label: "重试",
  ...
  saveError: (type: string, error: string) => showNotification.error({
    message: `保存${type}失败`,
    description: error
  }),

  // 删除相关消息
  deleteSuccess: (type: string, name?: string) => showMessage.success(
    name ? `${type} "${name}" 已删除` : `${type}已删除`
  ),
  deleteError: (type: string, error: string) => showNotification.error({
    message: `删除${type}失败`,
    description: error
  }),

  // 系统相关消息
  updateAvailable: (version: string) => showNotification.info({
    message: "发现新版本",
  ...
  systemError: (error: string) => showNotification.error({
    message: "系统错误",
    description: error,
    important: true,
    action: {
  ...
  // 数据相关消息
  dataImportSuccess: (count: number) => showNotification.success({
    message: "数据导入完成",
    description: `成功导入 ${count} 条记录`
  }),

  dataImportError: (error: string) => showNotification.error({
    message: "数据导入失败",
    description: error,
    action: {
      label: "查看详情",
