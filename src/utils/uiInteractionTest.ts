...
class UIInteractionTester {
    ...
    for (let i = 0; i < allToolbarButtons.length; i++) {
      ...
      try {
        ...
        this.recordTest(
          ...
          true,
          urlChanged
            ? `点击成功，路由从 ${beforeUrl} 变为 ${afterUrl}`
            : '点击成功但路由未变化',
          {
            buttonText,
            beforeUrl,
    ...
    for (const shortcut of shortcuts) {
      try {
        ...
        this.recordTest(
          ...
          true,
          urlChanged
            ? `快捷键生效，路由从 ${beforeUrl} 变为 ${afterUrl}`
            : '快捷键响应正常',
          {
            beforeUrl,
            afterUrl,
  ...
  async testModalsAndDialogs(): Promise<void> {
    ...
    const modalTriggers = document.querySelectorAll(
      '[data-testid*="modal"], [data-testid*="dialog"], button[class*="modal"], button[class*="dialog"]'
    );
