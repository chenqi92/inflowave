...
import { featureTester, runFeatureTests } from './featureTest';
...
class MasterTestRunner {
   ...
   * 计算功能测试统计
   */
  private calculateFeatureStats(featureResults: any[]): { total: number; passed: number; failed: number } {
    let total = 0;
    let passed = 0;
    let failed = 0;
  ...
  private printFinalReport(report: MasterTestReport): void {
    ...
    // 功能测试统计
    const featureStats = this.calculateFeatureStats(report.testSuites.featureTests);
    console.log('\n🔧 功能测试:');
    console.log(`   总计: ${featureStats.total}`);
    console.log(`   通过: ${featureStats.passed}`);
    console.log(`   失败: ${featureStats.failed}`);
    console.log(`   成功率: ${featureStats.total > 0 ? ((featureStats.passed / featureStats.total) * 100).toFixed(1) : 0}%`);
    ...
    console.log(`   成功率: ${((uiStats.passedTests / uiStats.totalTests) * 100).toFixed(1)}%`);
    
    // 详细分类统计
    console.log('\n📋 UI测试分类详情:');
    Object.entries(uiStats.categories).forEach(([category, stats]: [string, any]) => {
      console.log(`   ${category}: ${stats.passed}/${stats.total} (${((stats.passed / stats.total) * 100).toFixed(1)}%)`);
    });
    
    // 失败的测试列表
    const failedTests = uiStats.testResults.filter((test: any) => !test.success);
...
// 全局函数，可以在控制台调用
(window as any).runCompleteTests = () => masterTestRunner.runCompleteTestSuite();
