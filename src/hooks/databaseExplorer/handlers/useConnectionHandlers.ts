...
interface UseConnectionHandlersProps {
  ...
  setConnectionLoadingStates: React.Dispatch<
...
export const useConnectionHandlers = ({
  ...
  // ============================================================================
  const handleConnectionAndLoadDatabases = useCallback(
    async (connectionId: string) => {
      ...
      try {
        ...
        // 设置错误状态
        setConnectionErrors(prev =>
          new Map(prev).set(connectionId, errorMessage)
        );
  ...
  // Expand Connection
  // ============================================================================
  const handleExpandConnection = useCallback(
    async (connectionId: string) => {
      const connection = getConnection(connectionId);
      if (!connection) return;
  ...
  const handleConnectionToggle = useCallback(
    async (connection_id: string) => {
      ...
      startTransition(() => {
        setConnectionLoadingStates(prev =>
          new Map(prev).set(connection_id, true)
        );
