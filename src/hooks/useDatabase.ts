...
export const useDatabase = (connectionId?: string) => {
  ...
  const fetchDatabases = useCallback(
    async (connId?: string): Promise<DatabaseInfo[]> => {
      const id = connId || connectionId;
      if (!id) {
  ...
  const addDatabase = useCallback(
    async (name: string, connId?: string): Promise<void> => {
      const id = connId || connectionId;
      if (!id) {
        throw new Error('连接ID不能为空');
      }
  ...
  const getCurrentDatabase = useCallback(
    (connId?: string): string | undefined => {
      const id = connId || connectionId;
   ...
   * 获取测量列表
   */
  const getMeasurementList = useCallback(
    (database: string, connId?: string): MeasurementInfo[] => {
      const id = connId || connectionId;
      if (!id) return [];
  ...
  const fetchMeasurements = useCallback(
    async (database: string, connId?: string): Promise<MeasurementInfo[]> => {
      const id = connId || connectionId;
      if (!id) {
        throw new Error('连接ID不能为空');
      }
  ...
  const getFieldList = useCallback(
    (database: string, measurement: string, connId?: string): FieldInfo[] => {
      const id = connId || connectionId;
      if (!id) return [];
  ...
  const getTagList = useCallback(
    (database: string, measurement: string, connId?: string): TagInfo[] => {
      const id = connId || connectionId;
      if (!id) return [];
  ...
  const getSeriesList = useCallback(
    (database: string, measurement?: string, connId?: string): string[] => {
      const id = connId || connectionId;
      if (!id) return [];
  ...
  const fetchDatabaseStats = useCallback(
    async (database: string, connId?: string): Promise<any> => {
      const id = connId || connectionId;
      if (!id) {
        throw new Error('连接ID不能为空');
      }
  ...
  const refreshDatabaseStructure = useCallback(
    async (database?: string, connId?: string): Promise<void> => {
      const id = connId || connectionId;
      if (!id) return;
