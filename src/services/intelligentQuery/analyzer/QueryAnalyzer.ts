...
export class QueryAnalyzer {
  ...
  async getStatistics(
    ...
    const avgExecutionTime =
      filteredResults.reduce((sum, r) => sum + r.executionTime, 0) /
      totalQueries;
    const errorRate =
      filteredResults.filter(r => !r.success).length / totalQueries;
    ...
    const resourceUtilization =
      this.calculateResourceUtilization(filteredResults);
  ...
  private estimateResourceUsage(
    ...
    if (context?.dataSize) {
      const scaleFactor = Math.min(
        context.dataSize.totalSize / (1024 * 1024 * 1024),
        5
      );
  ...
  private checkWarnings(
    ...
    if (pattern.aggregations.length > 0 && pattern.groupBy.length === 0) {
      warnings.push(
        'Aggregation without GROUP BY may produce unexpected results'
      );
    }
  ...
  private extractJoins(query: string): Join[] {
    ...
    const joinMatches = query.match(
      /(?:inner|left|right|full|cross)?\s*join\s+([a-zA-Z_][a-zA-Z0-9_]*)/gi
    );
    if (joinMatches) {
  ...
  private extractAggregations(query: string): Aggregation[] {
    ...
    // 简化的聚合函数提取逻辑
    const aggMatches = query.match(
      /(count|sum|avg|min|max|first|last|mean|median|mode|stddev)\s*\([^)]*\)/gi
    );
    if (aggMatches) {
  ...
  private calculateResourceUtilization(
    ...
    const avgCpu =
      results.reduce((sum, r) => sum + r.executionTime / 1000, 0) /
      results.length;
    const avgIo =
      results.reduce((sum, r) => sum + r.diskReads + r.diskWrites, 0) /
      results.length;
    const avgNetwork =
      results.reduce((sum, r) => sum + r.networkBytes, 0) / results.length;
