...
export class PerformancePredictor {
    ...
    private linearRegressionPredict(
        ...
        // 计算性能提升预估
        const baselineDuration = duration * 1.5; // 假设基线性能
        const gain = Math.max(0, baselineDuration - duration);
    ...
    private decisionTreePredict(
        ...
        // 计算性能提升预估
        const baselineDuration = duration * 1.3; // 假设基线性能
        const gain = Math.max(0, baselineDuration - duration);
    ...
    private neuralNetworkPredict(
        ...
        // 计算性能提升预估
        const baselineDuration = output * 1.4; // 假设基线性能
        const gain = Math.max(0, baselineDuration - output);
    ...
    private fallbackPredict(
        ...
        // 计算性能提升预估
        const baselineDuration = duration * 1.2; // 假设基线性能
        const gain = Math.max(0, baselineDuration - duration);
