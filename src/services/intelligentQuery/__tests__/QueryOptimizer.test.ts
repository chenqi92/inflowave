...
describe('QueryOptimizer', () => {
  ...
  describe('optimize', () => {
    ...
    it('should apply rule-based optimizations', async () => {
      const query =
        "SELECT * FROM measurements WHERE time > '2023-01-01' ORDER BY time";

      const result = await optimizer.optimize(query, mockAnalysis, mockContext);

      expect(result.techniques.length).toBeGreaterThan(0);
      expect(
        result.techniques.some(
          t => t.name.includes('predicate') || t.name.includes('limit')
        )
      ).toBe(true);
    });
    ...
    it('should handle complex queries with joins', async () => {
      ...
      const query =
        'SELECT m.time, m.value FROM measurements m INNER JOIN metadata md ON m.id = md.id';
    ...
    it('should generate execution steps', async () => {
      ...
      expect(
        steps.every(step => step.id && step.operation && step.description)
      ).toBe(true);
    });

    it('should analyze parallelization opportunities', async () => {
      const steps = await optimizer.generateSteps(
    ...
    it('should calculate resource requirements', async () => {
      ...
      const requirements = optimizer.calculateResourceRequirements(
        steps,
        mockContext
      );
    ...
    it('should recommend indexes', async () => {
      ...
      const recommendations = await optimizer.recommendIndexes(
        query,
        mockAnalysis
      );
    ...
    it('should recommend query rewrites', async () => {
      const query =
        'SELECT DISTINCT time FROM measurements WHERE EXISTS (SELECT 1 FROM metadata WHERE metadata.id = measurements.id)';

      const recommendations = await optimizer.recommendRewrites(
        query,
        mockAnalysis
      );
