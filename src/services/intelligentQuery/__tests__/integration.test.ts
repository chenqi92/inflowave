...
describe('IntelligentQueryEngine Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    mockRequest = {
      query: 'SELECT time, value FROM measurements WHERE time > \'2023-01-01\' ORDER BY time',
  ...
  describe('Batch Query Optimization', () => {
    it('should optimize multiple queries', async () => {
      const requests = [
        ...
        {
          ...mockRequest,
          query: 'SELECT AVG(value) FROM measurements WHERE time > \'2023-01-01\''
        }
      ];
    ...
    it('should handle dependencies between queries', async () => {
      const dependentQueries = [
        {
          ...mockRequest,
          query: 'CREATE TEMP TABLE temp_data AS SELECT * FROM measurements WHERE time > \'2023-01-01\''
        },
        {
          ...mockRequest,
      ...
      const results = await intelligentQueryEngine.optimizeQueries(dependentQueries);
  ...
  describe('Learning and Feedback', () => {
    it('should learn from query execution results', async () => {
      ...
      await expect(
        intelligentQueryEngine.learnFromQuery(query, executionResult, mockContext)
      ).resolves.not.toThrow();
    });

    it('should provide query statistics', async () => {
      const stats = await intelligentQueryEngine.getQueryStats('test-connection-1');
    ...
    it('should provide optimization recommendations', async () => {
      const recommendations = await intelligentQueryEngine.getOptimizationRecommendations(
        'test-connection-1',
        5
      );
  ...
  describe('Cache Management', () => {
    ...
    it('should clear cache with pattern', async () => {
      await expect(intelligentQueryEngine.clearCache('test-*')).resolves.not.toThrow();
    });
  });
  ...
  describe('ML Integration', () => {
    ...
    it('should handle ML training data', () => {
      ...
      expect(() => intelligentQueryEngine.addMLTrainingData(trainingData)).not.toThrow();
    });

    it('should train ML models', async () => {
      await expect(intelligentQueryEngine.trainMLModels()).resolves.not.toThrow();
    });
  });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle missing connection gracefully', async () => {
      ...
      const result = await intelligentQueryEngine.optimizeQuery(noConnectionRequest);
